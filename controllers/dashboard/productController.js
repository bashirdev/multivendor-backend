...
//formidable version 2 use it here not newer version
...
class productController{
    ...
    //End Methode
    get_singleProduct=async(req,res)=>{
         ...
         if(err){
            ...
            try {
                ...
                if(result){
                    ...
                    const product=await productModel.findById(productId)
                    responseReturn(res, 200,{product, message : 'Product iamges Updated Successfully'})
                }else{
                    responseReturn(res, 404,{ error : 'Product Image update failed' })
                }
...
//     const form=formidable({multiples:true})
//     form.parse(req, async(err,field, files)=>{
//        let {name, price, brand, description, category, discount,stock, shopName} =field;
//        const images=files;
//        name = name.trim()
//        const slug =name.split(' ').join('-')
